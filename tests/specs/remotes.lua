...
local assert_buf_state = function(connected)
    ...
    local state = vesper.remote_state(term)
    assert((connected and state == 'connected') or state == 'disconnected', "The current terminal should be " .. ((connected and "connected") or "disconnected"))
end
...
local close_remote_buffer = function(remaining_buffers, callback)
    ...
    ERRORS.defer(TIMER, function()
        assert_buf_state(false)
        assert(#vesper.get_terminals() == remaining_buffers, "We should still have 3 terminals opened")
        assert(#get_channels() == remaining_buffers - 1, "We should still have 3 channels opened")
        t.simulate_keys('q', {PaneChanged = 1}, function()
            assert(#vesper.get_terminals() == remaining_buffers - 1, "We should now have 2 terminals opened")
            assert(#get_channels() == remaining_buffers - 1, "We should now have 2 channels opened")
            callback()
        end)
    end)
end
...
t.simulate_keys(s, {PaneChanged = 1}, function()
    ...
    local term = vesper.get_current_terminal()
    vesper.send_to_current('for run in {1..' .. LINES .. '}; do echo $run; done<cr>', true)
    ERRORS.defer(TIMER, function()
        t.wait_events({RemoteDisconnected = 1}, function()
            local terms = vesper.get_terminals()
            assert(#terms == 2, "We should have only 2 terminals")
            assert_buf_state(false)
            ERRORS.defer(1, function()
                assert(#get_channels() == 2, "We should have only 2 terminal channels opened")
                t.simulate_keys('r', {RemoteReconnected = 1}, function()
                    ERRORS.defer(TIMER, function()
                        assert(#get_channels() == 2, "After reconnecting, we should still have 2 channels opened")
                        ...
                        assert(found, "We should have in the current buffer a line containing " .. LINES)
                        ...
                        t.simulate_keys(s, {PaneChanged = 1}, function()
                            ...
                            t.simulate_keys(s, {PaneChanged = 1}, function()
                                t.simulate_keys(select_tab_shortcut('first'), {PaneChanged = 1}, function()
                                    ...
                                    ERRORS.defer(TIMER, function()
                                        assert(#get_channels() == 2, "We should have 2 opened channels")
                                        assert(#vesper.get_terminals() == 3, "We should have 3 opened terminals")
