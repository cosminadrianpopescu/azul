...
local assert_buf_state = function(connected)
    ...
    local chan = find_channel_by_id(term.term_id)
    local match = string.match(chan.argv[#chan.argv], 'sh$')
    t.assert((connected and (match ~= nil)) or (match == nil), "The current terminal should be " .. ((connected and "connected") or "disconnected"))
end
...
local close_remote_buffer = function(remaining_buffers, callback)
    ...
    vim.fn.timer_start(TIMER, function()
        ...
        t.assert(#get_channels() == remaining_buffers, "We should still have 3 channels opened")
