...
local assert_first_line = function(what)
    ...
    assert(line:match(what .. "$"), "The first line should end in " .. what)
end

t.wait_events({TabTitleChanged = 1}, function()
    local lines = funcs.reverse(t.get_current_term_lines())
    assert(lines[1] ~= "", "The first line should not be empty")
    t.simulate_keys("j")
                        ...
                        t.simulate_keys(s, {ModeChanged = (options.workflow == 'zellij' and 2) or 3, PaneChanged = 1}, function()
                            ERRORS.defer(TIMEOUT, function()
                                ...
                                ERRORS.defer(TIMEOUT, function()
                                    ...
                                    assert(lines[1] == lines[2], "The first two lines should be identical")
                                    ...
                                    t.simulate_keys(s, {ModeChanged = 1}, function()
                                        ERRORS.defer(TIMEOUT, function()
                                            ...
                                            t.simulate_keys(s, nil, function()
                                                assert(#vesper.get_terminals() == 2, "There should only be 2 terminals created")
                                                vesper.feedkeys("<C-\\><C-s>", 't')
                                                t.simulate_keys("<cr>", {ModeChanged = 1}, function()
                                                    t.simulate_keys(s, {PaneChanged = 1}, function()
                                                        assert(#vesper.get_terminals() == 3, "There should be now 3 terminals created")
                                                        t.done()
                                                    end)
                                                end)
                                            end)
