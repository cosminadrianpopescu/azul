...
local set_move_shortcuts = function(key, dir, inc)
    map('t', key, '', {
        callback = function()
            vesper.move_current_float(dir, inc or 5, false)
        end
    })
end

local set_panel_shortcuts = function(key, dir)
    set_hjkl_shortcuts(key, dir, vesper.select_next_pane)
end

local set_split_shortcuts = function(key, dir)
    set_hjkl_shortcuts(key, dir, vesper.split)
end

local set_position_shortcut = function(key, where)
    set_hjkl_shortcuts(key, where, vesper.position_current_float)
end
